...
public interface IMovieService
{
    ...
    Results.Bases.Result Add(MovieModel model);
    Results.Bases.Result Update(MovieModel model);

    [Obsolete("Do not use this method anymore, use DeleteUser method instead!")]
    Results.Bases.Result Delete(int id);
    Results.Bases.Result DeleteUser(int id);
...
public class MovieService : IMovieService // UserService is a IUserService (UserService implements IUserService)
{
    ...
    public IQueryable<MovieModel> Query()
    {
        ...
        return _db.Movies.Include(e => e.Director)
    ...
    public Results.Bases.Result Add(MovieModel model)
    {
        ...
        List<Movie> existingUsers = _db.Movies.ToList();
    ...
    public Results.Bases.Result Update(MovieModel model)
    {
        ...
        var existingUsers = _db.Movies.Where(u => u.Id != model.id).ToList();
        if (existingUsers.Any(u => u.Name.Equals(model.Name.Trim(), StringComparison.OrdinalIgnoreCase)))
            return new ErrorResult("Movie with the same  name already exists!");
    ...
    public Results.Bases.Result Delete(int id)
    {
        ...
        var userResourceEntities = _db.MoviesGenre.Where(ur => ur.MovieId == id).ToList();
        ...
        _db.MoviesGenre.RemoveRange(userResourceEntities);

        // 2) deleting the user record:
        var userEntity = _db.Movies.SingleOrDefault(u => u.Id == id);
    ...
    // Way 2: a better way
    public Results.Bases.Result DeleteUser(int id)
    {
        // getting the user record joined with the user resources records
        var userEntity = _db.Movies.Include(u => u.MovieGenres).SingleOrDefault(u => u.Id == id);
        if (userEntity is null)
            return new ErrorResult("User not found!");

        // 1) deleting the relational user resource records:
        _db.MoviesGenre.RemoveRange(userEntity.MovieGenres);
